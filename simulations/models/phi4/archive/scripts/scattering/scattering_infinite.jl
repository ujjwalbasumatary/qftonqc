...
function main()
    ...
    plt = heatmap(x, y, field_exp, xlabel=L"$n$", ylabel=L"$t$", title=L"$\langle\phi\rangle$ at $\tilde{\lambda} = %$λ0, \tilde{\mu}_0^2 = %$(μ0_sq)$", dpi=600)

    for t_step in 1:T-1
        println("Currently at step $t_step")
        ψ_window, _ = timestep(ψ_window, ham, t_step - 1, dt, TDVP())
        for i in 1:L
            field_exp[t_step+1, i] = real(expectation_value(ψ_window, i => ϕ))
        end
    ...
    mkpath("./plots_infinite/")
    mkpath("./data_infinite/")

    filename = replace("scattering_infinite_L_$(L)_T_$(T)_mu0sq_$(μ0_sq)_d_$(d)_D_$(D)_dt_$(dt)", '.' => 'p', '-' => 'm')
    full_path_image = "./plots_infinite/" * filename * ".png"

    savefig(plt, full_path_image)
    println("Plot saved.")

    full_path_data = "./data_infinite/" * filename * ".jld2"

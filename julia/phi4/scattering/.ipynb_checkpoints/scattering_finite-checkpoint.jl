...
function main()
    ...
    heatmap(x, y, field_exp, dpi=600, xlabel=L"Lattice position $n$", ylabel=L"Lattice time $t$")
    ...
    savefig("./plots/scattering_finite_L_$(L)_T_$(T)_musq_$(μ0_sq)_d_$(d)_D_$(D)_dt_$(dt).png")
    println("Plot saved.")

    if !isdir("./data")
        mkdir("./data")
    end
    @save "./data/scattering_finite_L_$(L)_T_$(T)_musq_$(μ0_sq)_d_$(d)_D_$(D)_dt_$(dt).jld2" field_exp
    println("Data saved.")
end
